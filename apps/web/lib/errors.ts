...
const sanitizeLog = (data: any) => {
  ...
  return str.replace(/(api[_-]?key|token|password|secret)["']?\s*[:=]\s*["']?[^"',}\s]+/gi, "$1=REDACTED");
};

export function handleApiError(error: unknown, requestId?: string): NextResponse {
  console.error(`[${requestId || "UNKNOWN"}]`, sanitizeLog({
  ...
  if (error instanceof z.ZodError) {
    return NextResponse.json(
      { error: "Invalid input data", code: "VALIDATION_ERROR", details: error.errors[0]?.message },
      { status: 400 }
    );
  }
